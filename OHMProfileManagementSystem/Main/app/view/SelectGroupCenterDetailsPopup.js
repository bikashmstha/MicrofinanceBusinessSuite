/*
 * File: app/view/SelectGroupCenterDetailsPopup.js
 *
 * This file was generated by Sencha Architect version 4.2.2.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 4.1.x library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 4.1.x. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('MyApp.view.SelectGroupCenterDetailsPopup', {
    extend: 'Ext.window.Window',

    requires: [
        'Ext.form.Panel',
        'Ext.form.field.Text',
        'Ext.button.Button',
        'Ext.grid.Panel',
        'Ext.grid.View',
        'Ext.grid.column.Column',
        'Ext.selection.RowModel'
    ],

    height: 500,
    itemId: 'SelectGroupCenterDetailsPopup',
    width: 700,
    autoScroll: true,
    resizable: false,
    layout: 'fit',
    title: 'Select Center',
    maximizable: true,
    minimizable: true,
    modal: true,
    y: 80,

    initComponent: function() {
        var me = this;

        Ext.applyIf(me, {
            items: [
                {
                    xtype: 'form',
                    frame: true,
                    autoScroll: true,
                    bodyPadding: 10,
                    title: '',
                    items: [
                        {
                            xtype: 'container',
                            layout: 'table',
                            items: [
                                {
                                    xtype: 'textfield',
                                    itemId: 'txtSearchCenterOfficeCode',
                                    fieldLabel: 'Office Code'
                                },
                                {
                                    xtype: 'textfield',
                                    itemId: 'txtSearchCenterName',
                                    fieldLabel: 'Center Name'
                                },
                                {
                                    xtype: 'button',
                                    colspan: 2,
                                    itemId: 'btnSearchCenter',
                                    margin: '0 0 0 5',
                                    text: 'Search',
                                    listeners: {
                                        click: {
                                            fn: me.onBtnSearchCenterClick,
                                            scope: me
                                        }
                                    }
                                }
                            ]
                        },
                        {
                            xtype: 'gridpanel',
                            itemId: 'grdSearchCenter',
                            title: '',
                            store: 'GroupCenterDetailsSearchStore',
                            columns: [
                                {
                                    xtype: 'gridcolumn',
                                    dataIndex: 'CenterCode',
                                    text: 'Center Code'
                                },
                                {
                                    xtype: 'gridcolumn',
                                    width: 189,
                                    dataIndex: 'CenterName',
                                    text: 'Center Name'
                                },
                                {
                                    xtype: 'gridcolumn',
                                    dataIndex: 'EmployeeId',
                                    text: 'Employee Id'
                                },
                                {
                                    xtype: 'gridcolumn',
                                    width: 263,
                                    dataIndex: 'EmployeeName',
                                    text: 'Employee Name'
                                }
                            ],
                            selModel: Ext.create('Ext.selection.RowModel', {
                                listeners: {
                                    beforeselect: {
                                        fn: me.onRowModelBeforeSelect,
                                        scope: me
                                    }
                                }
                            })
                        }
                    ]
                }
            ],
            listeners: {
                afterrender: {
                    fn: me.onSelectGroupCenterDetailsPopupAfterRender,
                    scope: me
                }
            }
        });

        me.callParent(arguments);
    },

    onBtnSearchCenterClick: function(button, e, eOpts) {

        var officeCode=Ext.ComponentQuery.query('#txtSearchCenterOfficeCode')[0];
        var centerName=Ext.ComponentQuery.query('#txtSearchCenterName')[0];
        var centerobj={

            OfficeCode :officeCode.getValue(),
            CenterName: centerName.getValue()

        };
        var waitSave = watiMsg('Loading Data. Please wait ...');
        Ext.Ajax.request({
            url: '../Handlers/GeneralMasterParameters/Center/CenterHandler.ashx',
            params: {
                method:'Search', center:JSON.stringify(centerobj)
            },
            success: function(response){


            var obj = Ext.decode(response.responseText);

              if(obj.success === "true")
              {

                  var store=Ext.getStore('GroupCenterDetailsSearchStore');
                  store.removeAll();
                  store.add(obj.root);

              }
                else
                {

                    msg("FAILURE",obj.message);
                }


            waitSave.hide();



            }
        });
    },

    onRowModelBeforeSelect: function(rowmodel, record, index, eOpts) {
        Ext.MessageBox.confirm('Select', 'Are you sure ?', function(btn){

           if(btn === 'yes'){


               Ext.ComponentQuery.query('#txtSearchCenterDetailsCode')[0].setValue(record.data.CenterCode);
               Ext.ComponentQuery.query('#txtSearchCenterDetailsName')[0].setValue(record.data.CenterName);


              // return true;
           }
           else
           {

           }
            var v=Ext.ComponentQuery.query('#SelectGroupCenterDetailsPopup')[0];

            v.close();
        });

    },

    onSelectGroupCenterDetailsPopupAfterRender: function(component, eOpts) {
        var store=Ext.getStore('GroupCenterDetailsSearchStore');
        store.removeAll();
    }

});