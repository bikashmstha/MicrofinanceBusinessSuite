...
namespace IntegratedTaxSystem.Controllers.Security
{
    public class LoginController : ControllerBase
    {
        ...
        public MenuData GetMenuData(List<Menu> menuList)
        {
            ...
            foreach (Menu menu in menuList)
            {
                if (menu.AppID != applicationId)
                {
                    ...
                    List<Menu> modules = menuList.FindAll(delegate(Menu mnu) { return mnu.AppID == applicationId; });
                    ...
                    foreach (Menu mdl in modules)
                    {
                        ...
                        {
                            hasSubModule = !string.IsNullOrEmpty(mdl.SubModuleDesc);
                            ...
                            if (!hasSubModule)
                            {
                                ...
                                module.leaf = false;
                                List<Menu> forms = menuList.FindAll(delegate(Menu mnu) { return mnu.ModuleID == moduleId && mnu.AppID == applicationId; });
                            ...
                            else
                            {
                                ...
                                List<Menu> subModules = menuList.FindAll(delegate(Menu mnu) { return mnu.AppID == applicationId && mnu.ModuleID == moduleId; });
                                ...
                                foreach (var sModule in subModules)
                                {
                                    ...
                                    if (sModule.SubModuleID != subModId)
                                    {
                                        ...
                                        List<Menu> forms = menuList.FindAll(delegate(Menu mnu) { return mnu.ModuleID == moduleId && mnu.AppID == applicationId && mnu.SubModuleID == subModId; });
                                        string formID = string.Empty;
                                        if (modules.Count > 0)
                                        {
                                            foreach (Menu form in forms)
        ...
        //public MenuData GetMenuData(List<Menu> menuList) // 2nd version
        //{
        //    MenuData menudata = new MenuData();
        //    menudata.text = "MicroFinance";
        //    bool hasSubModule = false;
        ...
        //public MenuData GetMenuData(List<Menu> menuList) //1st version used in ird
        //{
        //    MenuData menudata = new MenuData();
        //    menudata.text = "MicroFinance";
